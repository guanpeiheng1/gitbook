...
在docs新建文件夹.vuepress，文件夹内新建config.js:
...
module.exports = {
  title: 'Payton\'s blog',
  description: 'Somewhere Leisure.',
  base: '/myblog/',
  plugins: ['autobar'],
  themeConfig: {
    logo: 'icon.png',
    repo: 'paytonguan/myblog',
    smoothScroll: true,
    sidebarDepth: 2,
...
同时文件夹内新建文件夹public，放入icon.png，在左上角显示图标

根目录的README.md(可以将相应的内容设置为 null 来禁用标题和副标题)，在上面的public文件夹放入image.jpeg作为主图：

```
---
home: true
heroImage: image.jpeg
heroText: Payton's Blog
tagline: Somewhere leisure.
actionText: Enter →
actionLink: /nav.20.Technology/
features:
...
文章的组织：根目录的文件夹是导航栏上的大目录，以`nav.`开头，10/20/30表示次序（先显示10，再20），如nav.10.filename

这个大目录内必须要有一个README.md，作为点击导航栏上目录时显示的文件

目录内可以再放文件夹（作为组，在侧边栏出现），也可以直接放md文件（也出现在侧边栏），文件夹内不需要再放README.md了
...
如果仓库名是`[用户名].github.io`，base为`/`，如果是随意的，则为`'[repo名]'`

在根目录新建以下脚本deploy.sh，终端运行即可部署：
